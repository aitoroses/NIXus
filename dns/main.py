...
env = Environment(loader=PackageLoader('assets', 'templates'))
POLL_TIMEOUT=5
...
def generate_config(services):
    # Generate redbird reverse proxy
    template = env.get_template('redbird.js.tmpl')
    with open("/etc/proxy.js", "w") as f:
